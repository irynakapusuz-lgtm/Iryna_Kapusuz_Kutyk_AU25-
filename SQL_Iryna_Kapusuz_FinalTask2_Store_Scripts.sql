...
CREATE OR REPLACE FUNCTION add_order(customer_email TEXT, employee_id INT,
...
RAISE EXCEPTION 'Customer not found with email: %', customer_email;
END IF;
INSERT INTO store.Orders (customer_id, employee_id, order_date, status)
VALUES (cust_id, employee_id, order_date, status);
END;
$$ LANGUAGE plpgsql;
...
GRANT CONNECT ON DATABASE household_appliances_store TO manager;
